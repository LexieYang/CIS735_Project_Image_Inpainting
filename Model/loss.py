...
class ConsisLoss(nn.Module):
    ...
    def forward(self, in_data, in_target, in_mask):
        batch_size, ch_dim, _, _ = in_data.size()
        self.former = in_data
        self.former_in_mask = self.former * in_mask
        

        self.loss = self.criterion(self.former_in_mask * self.strength, in_target)
        return self.loss
